...
4. Deploy — the launcher starts NodeLink, waits for its API, then starts the bot.
   ...
   as your health-check / uptime-monitor URL. The page also shows the deployed
   build SHA (from `RENDER_GIT_COMMIT`), so `curl -s <service-url> | grep build`
...
- **Pagination buttons never expire.** `pgn:` arrows are answered by a real component handler (`src/components/pagination.js`) that rebuilds the page from live state, instead of a message collector — a collector's `update()` always lost the race against the router's "unknown interaction" reply, which made page arrows look dead. Queue, history, lyrics, leaderboard, warnings and mod-logs pages all keep working for as long as the bot runs.
- **Poll votes** are answered by a real `poll:` component handler (`src/components/polls.js`) instead of a message collector — same story as the pager: the router answers every component interaction first, so votes kept bouncing with the generic "expired or unknown" error. Voting is single-choice with instant re-voting, confirmed in an ephemeral reply, and results are posted by a timer (plus a lazy finalize on post-deadline clicks) when the deadline passes. Tally state is in-memory: a restart mid-poll drops it and clicks say so.
- **Never put `V2_FLAG` on `deferReply()`.** Discord rejects `IS_COMPONENTS_V2` on a deferred callback (only `MessageFlags.Ephemeral` is allowed there) and the bot only surfaces a generic "something went wrong" reply, so `npm run check` now fails on that pattern — defer with `MessageFlags.Ephemeral` and let `editReplyV2()`/`paginate()` set the V2 flag on the edit.
